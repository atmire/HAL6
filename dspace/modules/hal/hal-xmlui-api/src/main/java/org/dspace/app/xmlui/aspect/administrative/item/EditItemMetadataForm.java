...
public class EditItemMetadataForm extends AbstractDSpaceTransformer {
    ...
    private static final Message T_remove = message("xmlui.administrative.item.EditItemMetadataForm.remove");

    private static final Message T_child_value_header = message("xmlui.administrative.item.EditItemMetadataForm.child-value.header");
    private static final Message T_parent_value_header = message("xmlui.administrative.item.EditItemMetadataForm.parent-value.header");
    private static final Message T_manage_author = message("xmlui.hal.manage.author");
    ...
    public void addBody(Body body) throws SQLException, WingException
    {
        ...
        org.dspace.app.xmlui.wing.element.Item hiddenFunctionSelection = addForm.addItem("hiddenFunctions","hidden");
        ...
        {
            String metadataField = value.getMetadataField().toString('.');
            if (hiddenMetadataFields.contains(metadataField)) {
                continue;
            }
            ...
            remove.setLabel(T_remove);
            remove.addOption(index);

            Cell cell = row.addCell("nameCell",null,"preWrapWhiteSpace");
            ...
            java.util.List<String> structureIdsList = new ArrayList<>();
            if(!CollectionUtils.isEmpty(value.getChildMetadataValues())){
                cell.addContent(T_child_value_header);
                for(MetadataValue metadataValue: value.getChildMetadataValues()){
                    MetadataField childMetadataField = metadataValue.getMetadataField();
                    if(childMetadataField.equals(structureMetadataField)){
                        Structure structureByID = halRetrievalService.findStructureByID(Integer.parseInt(metadataValue.getValue()));
                        if (structureByID != null) {
                            cell.addContent("\nStructure: " + structureByID.getId() + " " + structureByID.getName());
                            structureMetadataValueList.add(metadataValue);
                            structureIdsList.add(metadataValue.getValue());
                        }

                    } else if(childMetadataField.equals(halFunctionMetadataField)){
                        cell.addContent("\nFunction: " + halRetrievalService.findFunction(metadataValue.getValue()));
                        cell.addHidden("name_"+index+"_functionValue").setValue(metadataValue.getValue());
                    }
                }
            }
            if (managedMetadataFIeld.contains(value.getMetadataField().toString('.'))) {
                String idAndNameCombo = HALConversionUtils.convertStructureIdsStringToHTML(structureMetadataValueList, "$$$", true);
                cell.addHidden(value.getMetadataField().toString() + "_structure_IDAndName_" + index, "hidden").setValue(idAndNameCombo);
                cell.addHidden("valueID").setValue(String.valueOf(value.getID()));
                cell.addHidden("name_" + index + "_structureValues").setValue(StringUtils.join(structureIdsList, "$$$"));
                cell.addContent("\n");
                cell.addButton("edit-author-manage_"+index, "btn btn-default ").setValue(T_manage_author);
            }
            if(value.getParentMetadataValue()!= null){
                MetadataValue parentValue = value.getParentMetadataValue();
                String parentMetadataField = parentValue.getMetadataField().toString('.');
                String parentStringValue = parentValue.getValue();
                cell.addContent(T_parent_value_header.parameterize("\t"+parentMetadataField +": "+ parentStringValue));
                cell.addHidden("name_"+index+"_parentValue").setValue(parentMetadataField +":"+ parentStringValue);
