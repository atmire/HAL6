...
public class FlowItemUtils 
{
	...
	protected static final AtmireItemService itemService = (AtmireItemService) ContentServiceFactory.getInstance().getItemService();
	...
	protected static final AtmireMetadataValueService metadataValueService =  (AtmireMetadataValueService)ContentServiceFactory.getInstance().getMetadataValueService();
            ...
            throws SQLException, AuthorizeException, UIException, IOException
	{
		...
		while(parameters.hasMoreElements())
		{
			...
			if (parameterName.startsWith("name_") && !StringUtils.endsWith(parameterName,"structureValues") && !StringUtils.endsWith(parameterName,"_functionValue") && !StringUtils.endsWith(parameterName,"_parentValue"))
			{
		...
		for (Integer index = 1; index <= indexes.size(); ++index) {
            ...
            String structureValues = request.getParameter("name_"+index+"_structureValues");
            String functionValue = request.getParameter("name_"+index+"_functionValue");

			itemService.addMetadata(context, item, parts[0], parts[1], parts[2], lang,
					value, authority, iconf);

			if (StringUtils.isNotBlank(structureValues) || StringUtils.isNotBlank(functionValue)) {
				// We just added the value above, but now we need the actual value, just retrieve the last one in the list
				MetadataValue authorMetadataValue = Iterables.getLast(itemService.getMetadata(item, parts[0], parts[1], parts[2], lang), null);
				if (authorMetadataValue != null) {
					if (StringUtils.isNotBlank(structureValues)) {
						for (String structureId : StringUtils.split(structureValues, "$$$")) {
							itemService.addMetadata(context, item, "hal", "structure", "identifier", null, structureId);
							MetadataValue structureValue = Iterables.getLast(itemService.getMetadata(item, "hal", "structure", "identifier", null), null);
							metadataValueService.addChildMetadataValue(context, item, authorMetadataValue, structureValue);
						}
					}
					if (StringUtils.isNotBlank(functionValue)) {
						itemService.addMetadata(context, item, "hal", "author", "function", null, functionValue);
						MetadataValue functionMetadataValue = Iterables.getLast(itemService.getMetadata(item, "hal", "author", "function", null), null);
						metadataValueService.addChildMetadataValue(context, item, authorMetadataValue, functionMetadataValue);
					}
	...
	private static void updateRelations(Context context, Item item, Map.Entry<MetadataValue, String> entry, String value, List<MetadataValue> parentValueList) throws AuthorizeException, SQLException {
		Iterator<MetadataValue> iterator = parentValueList.iterator();
		boolean parentFound = false;
		while (iterator.hasNext() && !parentFound) {
            MetadataValue possibleParent = iterator.next();
            if (StringUtils.equals(possibleParent.getValue(), value)) {
                parentFound = true;
                metadataValueService.addChildMetadataValue(context, item, possibleParent, entry.getKey());
            }
        }
		if(!parentFound){
            itemService.removeSingleMetadataValue(context, item, entry.getKey(), true);
        }
	}
