...
public abstract class DSpaceObjectServiceImpl<T extends DSpaceObject> implements DSpaceObjectService<T> {
    ...
    public List<MetadataValue> getMetadata(T dso, String schema, String element, String qualifier, String lang) {
        List<MetadataValue> metadata = dso.getMetadata();
        if(StringUtils.equals(schema, Item.ANY) && StringUtils.equals(element, Item.ANY) && StringUtils.equals(qualifier, Item.ANY) && StringUtils.equals(lang, Item.ANY)){
            // Simply return all metadata when all checking variables equal Item.ANY (*),
            // since they are all returned anyway and this can skip looping through all values
            return metadata;
        }
        // Build up list of matching values
        List<MetadataValue> values = new ArrayList<MetadataValue>();
        for (MetadataValue dcv : metadata)
        {
...
//            metadataValueService.update(context, metadataValue);
            ...
            String language, MetadataValue metadataValue)
    {
        ...
        if (language == null)
        {
            ...
            if (StringUtils.isNotBlank(metadataValue.getLanguage()))
            {
                // Value is qualified, so no match
                return false;
            }
