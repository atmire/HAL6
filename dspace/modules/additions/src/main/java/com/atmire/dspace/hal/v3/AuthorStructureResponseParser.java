...
/**
 * a hack of XMLResponseParser...
 * (compare with XMLResponseParser to see the customizations)
 *
 */
public class AuthorStructureResponseParser extends ResponseParser
{
  public static Logger log = LoggerFactory.getLogger(AuthorStructureResponseParser.class);
  ...
  public AuthorStructureResponseParser() {}
  ...
  protected enum KnownType {
    ...
    DATE   (true)  {
      ...
      public Date read( String txt ) {
        try {
          return DateUtils.parseDate(txt, "yyyy", "yyyy-MM", "yyyy-MM-dd");
        }
        catch( Exception ex ) {
    ...
    public static KnownType get( String v )
    {
      if( v != null ) {
        ...
        return STR; // accept anything as a STR so it won't crash (only leafs will be added though)
    }
...
//    }
  ...
  protected SolrDocumentList readDocuments(XMLStreamReader parser ) throws XMLStreamException
  {
    ...
    while( true ) {
      ...
      if( XMLStreamConstants.START_ELEMENT == event ) {
        if( !("doc".equals( parser.getLocalName()) || "org".equals( parser.getLocalName() )) ) {
  ...
  protected SolrDocument readDocument(XMLStreamReader parser ) throws XMLStreamException
  {
    ...
    if( !("doc".equals( parser.getLocalName().toLowerCase(Locale.ROOT) ) || "org".equals( parser.getLocalName().toLowerCase(Locale.ROOT)))) {
...
//        }
          ...
          case XMLStreamConstants.END_ELEMENT:
              ...
              Object val = null;
              if (nameVal == null) {
                  val = type.read(builder.toString().trim());
                  if (val == null) {
                      throw new XMLStreamException("error reading value:" + type, parser.getLocation());
                  }
              }else {
    ...
    private String getID(XMLStreamReader parser) {
        String id = null;
        int count = parser.getAttributeCount();
        for (int i = 0; i < count; i++) {
            if ("id".equals(parser.getAttributeLocalName(i))) {
